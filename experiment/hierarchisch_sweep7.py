"""Vervolg op hierarchisch_sweep6.py: breedte en diepte zijn nu aan beide
kanten uitputtend verkend (experiment 20-22), geen winst meer daar. Volgende
open punt uit EXPERIMENTEN.md experiment 16: BROK_VENSTER (nu 32 brokken,
~64 karakters) is nooit los getest - gewoon overgenomen om qua karakters te
matchen met het char-model se LENGTE=64. Bij het char-model was de
venstergrootte destijds de waardevolste knop van allemaal (zie exp.py's eigen
commentaar bij LENGTE) - dus de moeite waard om hier ook te proberen.

Winnende architectuur (n_embed_binnen=128, 6 lagen, n_embed_buiten=160, 5
lagen, lr=3e-3) blijft vast; alleen BROK_VENSTER varieert. MAX_BROK_LENGTE
blijft 16, dus dezelfde brok-tensor kan hergebruikt worden.
"""
import time
...
WINNAAR = 1.1527  # BROK_VENSTER=32 - zie sweep5/6

VARIANTEN_VENSTER = [16, 48]
...
UITVOER = Path(__file__).parent / "hierarchisch_sweep7_resultaten.pt"
...
print("\n" + "=" * 70)
print(f"{'':>22s}  nats/char   verschil vs winnaar (BROK_VENSTER=32: 1,1527)")
print(f"{'char-model':>22s}  {BASELINE_CHAR:>9.4f}")
print(f"{'BROK_VENSTER=32':>22s}  {WINNAAR:>9.4f}   (referentie)")
for brok_venster in VARIANTEN_VENSTER:
    naam = f"BROK_VENSTER={brok_venster}"
    r = resultaten[naam]
    print(f"{naam:>22s}  {r['nats_excl']:>9.4f}   {r['nats_excl']-WINNAAR:>+9.4f}")

scores = {"BROK_VENSTER=32": WINNAAR}
for brok_venster in VARIANTEN_VENSTER:
    naam = f"BROK_VENSTER={brok_venster}"
    scores[naam] = resultaten[naam]["nats_excl"]
beste_naam = min(scores, key=scores.get)
print(f"\nbeste: {beste_naam}  ({scores[beste_naam]:.4f})")
if beste_naam != "BROK_VENSTER=32" and scores[beste_naam] < WINNAAR:
    print("NIEUWE WINNAAR gevonden!")

fig, ax = plt.subplots(figsize=(9, 6))
for brok_venster, kleur in zip(VARIANTEN_VENSTER, ["tab:olive", "tab:pink"]):
    naam = f"BROK_VENSTER={brok_venster}"
    r = resultaten[naam]
...
ax.axhline(WINNAAR, color="black", linestyle=":", label=f"BROK_VENSTER=32 eindresultaat ({WINNAAR})")
ax.set_ylim(0, 3)
...
fig.tight_layout()
plot_pad = Path(__file__).parent / "hierarchisch_sweep7.png"
